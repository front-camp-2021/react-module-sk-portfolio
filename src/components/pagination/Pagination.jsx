...
export function Pagination(props) {
    ...
    const numberOfProducts = props.numderOfPages - 1
    ...
    return (
        <nav className="products-list__pagination pagination" data-element="wrapper">
            ...
            <ul className="pagination__list" data-element="list">
                {
                    setPageNumbers(numberOfProducts).map(number => <PaginationLink key={number} id={number} currentId={indexPage.id}/>)
                }
            </ul>
            <button className="pagination__btn pagination__btn--right" data-element="next" onClick={() => correctNextId(indexPage.id, 1)}>
